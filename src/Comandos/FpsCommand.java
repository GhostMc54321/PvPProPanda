...
public class FpsCommand extends WarpsAPI implements CommandExecutor {
		...
		if (KitAPI.getKit(p) == "Nenhum") {
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstace(), new Runnable() {
				...
				public void run() {
					...
					p.sendMessage("§5§lEPIC§F§lKITS§c ➜ §aVocê Entrou na arena §fFPS");
			...
			if (args[0].equalsIgnoreCase("set")) {
			    if (p.hasPermission("pluspvp.cmd.setarenas")) {
			    	Set(p, "Fps");
			    	p.sendMessage("§5§lEPIC§F§lKITS§c ➜ §aVocê setou a arena §fFPS");
			}
		  }
		}
		return false;
